(function () {
  ...
  function updateUI() {
    ...
    const cartCountStripEl = document.getElementById('cartCountStrip');
    if (cartCountStripEl) {
      cartCountStripEl.textContent = count;
      cartCountStripEl.classList.toggle('visible', count > 0);
    }
    ...
    ordersPollTimer = setInterval(async () => {
      ...
      for (const order of orders) {
        ...
        if (idx >= 0 && idx < STATUS_STEPS.length - 1) {
          const result = await advanceOrderStatus(order.id, order.status);
          if (result.advanced) {
            anyChanged = true;
            showToast(toastEl, 'Order #' + order.id.slice(0, 8) + ' is now ' + result.nextStatus);
            lastOrdersStatus[order.id] = result.nextStatus;
          }
        }
      }
        ...
        <div class="order-card" data-id="${order.id}">
          <div class="order-card-header">
            <div>
              ...
              <span class="order-date">${date} &middot; ${qty} item${qty !== 1 ? 's' : ''}</span>
            </div>
    ...
    orders.forEach(order => {
      ...
      if (placeholder) {
        ...
        renderStatusFlow(placeholder, order.status, {
          meta: ['', `$${parseFloat(order.total).toFixed(2)} ${qty} item${qty !== 1 ? 's' : ''}`, '', '', ''],
        });
      }
    });
  }
  ...
  const mobileTabStrip = document.getElementById('mobileTabStrip');
  if (mobileTabStrip) {
    mobileTabStrip.addEventListener('click', function (e) {
      const btn = e.target.closest('.mts-tab');
