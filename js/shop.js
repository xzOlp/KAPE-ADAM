    ...
    grid.innerHTML = data.map(item => {
      ...
      return `
        <div class="menu-card" data-id="${item.id}">
          <img class="menu-card-img" src="${item.img}" alt="${item.name}" loading="lazy" onerror="this.style.display='none';this.parentElement.querySelector('.menu-card-img-fallback')?.classList.remove('menu-card-img-fallback-hidden')" /><div class="menu-card-img-fallback menu-card-img-fallback-hidden">${item.name.charAt(0)}</div>
